...
<link rel="stylesheet" type="text/css" href="/IKA/assets/css/cert_ins.css" media="screen" />
...
?>
  ...
  $afm = $rows['AFM'];
  $sql = "SELECT *, DATE_FORMAT(INSUR_DATE, '%d/%m/%Y') as MY_INSUR_DATE FROM insurance_info WHERE AFM='$afm' ";
  $result = $mysqli->query( $sql );
  $rows2 = $result->fetch_assoc();

  if ( $rows2['INSUR_TYPE'] == 0) {
  ?>
  <div class="print">
    <h1 class="title">Επίσημη Βεβαίωση Εργατοωρών</h1>
    <p class="text">Βεβαιώνεται πως ο <?php echo $rows['NAME'], " ", $rows['SURNAME']; ?> έχει συμπληρώσει <?php echo $rows2['WORKHOURS']; ?> εργατοώρες, στο χρονικό </br>διάστημα που είναι ασφαλισμένος, από τις <?php echo $rows2['MY_INSUR_DATE']; ?> μέχρι και σήμερα <?php echo date("d/m/Y"); ?>!</p>
  </div>
  ...
  <?php } else { ?>
  <div class="print">
    <h1>Πρέπει να είσαι ασφαλισμένος για να λάβεις αυτή τη βεβαίωση!</h1>
  </div>
